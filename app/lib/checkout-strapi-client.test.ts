...
test("simulatePayment: propaga erro ORDER_NOT_PENDING", async (t) => {
  process.env.STRAPI_INTERNAL_URL = "http://strapi.internal";
  const { simulatePayment } = await import("./checkout-strapi-client");

  t.mock.method(globalThis, "fetch", async () => jsonResponse({ ok: false, error: "ORDER_NOT_PENDING" }, 409));

  const result = await simulatePayment("token-abc", "abc123def4");
  assert.deepEqual(result, { ok: false, error: "ORDER_NOT_PENDING", status: 409 });
});
