...
export default async function CategoriasPage() {
  const categories = visibleCategories(await getCategories());

  return (
    <section className="max-w-310 my-0 mx-auto py-10 px-6 w-full">
      <Breadcrumb items={[{ label: "Início", href: "/" }, { label: "Categorias" }]} />
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(240px,1fr))] gap-4">
        {categories.map((c, i) => (
          ...
          >
            ...
            <span className="font-bold text-vh-12 font-space-grotesk text-vh-lime">{plural(c.productCount, "produto")} →</span>
          </Link>
        ))}
      </div>
    </section>
