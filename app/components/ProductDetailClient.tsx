...
const WHATSAPP_CHANNEL_URL = "https://whatsapp.com/channel/0029Vag8CGyKgsNvjnjeE03j";
...
export default function ProductDetailClient({
  ...
  const configLabels = Array.from(
    new Set(product.variants.map((v) => v.configLabel)),
  );
  ...
  const selectedVariant =
    product.variants.find(
      (v) => v.color.name === selColorName && v.configLabel === selConfigLabel,
    ) ?? product.variants[0];

  const allUnavailable = isSoldOut(product);
  ...
  const pBadge = selectedVariant?.compareAtPrice
    ? "-" +
      Math.round(
  ...
  const pOldPriceF = selectedVariant?.compareAtPrice
    ? fmt(selectedVariant.compareAtPrice)
    : null;
    ...
    <section className="max-w-310 my-0 mx-auto py-9 px-6 w-full">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] gap-11 items-start">
        <div className="flex flex-col gap-3">
          <div className="relative aspect-square bg-[repeating-linear-gradient(45deg,#2A0A45_0_14px,#24063C_14px_28px)] border border-vh-border rounded-vh-18 flex items-center justify-center">
            {selPhoto ? (
              ...
              <span className="font-medium text-vh-12 font-mono text-vh-muted bg-vh-ink/55 py-1.5 px-3 rounded-md">
                [foto principal: {product.name}]
              </span>
            )}
            {pBadge && (
              <span className="absolute top-3.5 left-3.5 font-bold text-vh-12 font-space-grotesk tracking-vh-006 uppercase py-1.5 px-2.75 rounded-vh-7 bg-vh-lime text-vh-ink">
                {pBadge}
              </span>
            )}
          </div>
          <div className="grid grid-cols-4 gap-2.5">
            {photos.slice(0, 8).map((ph, i) => (
              <button
                ...
                type="button"
                className={cn(
                  "vh-thumb relative aspect-square bg-vh-card rounded-vh-10 cursor-pointer [transition:border-color_.12s] p-0 overflow-hidden border",
                  i === selImageIdx ? "border-vh-lime" : "border-vh-border",
                )}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <span className="font-semibold text-vh-11-5 font-space-grotesk tracking-vh-014 uppercase text-vh-accent">
              {product.brand?.name}
            </span>
            <h1 className="m-0 font-bold text-[clamp(24px,3vw,32px)]/vh-115 font-space-grotesk">
              {product.name}
            </h1>
            <span className="font-medium text-vh-12 font-mono text-vh-muted">
              Cód. {pCode}
            </span>
          </div>
          <span
            className={cn(
              "self-start font-bold text-vh-11-5 font-space-grotesk tracking-vh-008 uppercase py-1.5 px-3 rounded-vh-20 border",
              bgClass(st[1]),
              textClass(st[2]),
              borderClass(st[3]),
            )}
          ...
          <div className="bg-vh-card border border-vh-border rounded-vh-14 py-4.5 px-5 flex flex-col gap-0.5">
            {pOldPriceF && (
              <span className="font-medium text-vh-14 font-manrope text-vh-muted line-through">
                {pOldPriceF}
              </span>
            )}
            <span className="font-bold text-vh-34 font-space-grotesk text-vh-lime">
              {fmt(unit)}
            </span>
            <span className="font-medium text-vh-12-5 font-manrope text-vh-muted">
              Preço à vista de referência · condições finais com o atendente
            </span>
          </div>
          <div className="flex flex-col gap-2">
            <span className="font-bold text-vh-12-5 font-space-grotesk tracking-vh-006 uppercase text-vh-soft">
              Cor: <span className="text-vh-lime">{selColorName}</span>
            </span>
            <div className="flex gap-2.5 flex-wrap">
              {colors.map((c, i) => {
                const unav = product.variants
                ...
                return (
                  <span
                    ...
                    className={cn(
                      "relative w-8.5 h-8.5 rounded-full [transition:box-shadow_.12s]",
                      unav
                        ? "cursor-not-allowed opacity-45"
                        : "cursor-pointer opacity-100",
                      ringShadowClass(
                        selected ? "#8CFF00" : unav ? "#22003D" : "#341052",
                      ),
                    )}
                  >
                    {unav && (
                      <span className="absolute -left-1 -right-1 top-1/2 h-0.5 bg-vh-muted -rotate-45"></span>
                    )}
                  </span>
                );
              })}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2.5 flex-wrap">
              {configLabels.map((label, i) => {
                const variantsForLabel = product.variants.filter(
                  (v) => v.configLabel === label,
                );
                ...
                return (
                  <span
                    ...
                    className={cn(
                      ...
                      unav
                        ? "cursor-not-allowed text-vh-muted line-through"
                        : "cursor-pointer",
                      sel
                        ? "border-vh-lime text-vh-lime bg-vh-lime/8"
                        : "border-vh-border text-white bg-transparent",
                    )}
                  >
                    {labelF}
                  </span>
            ...
            {pVarUnavailable && (
              <div className="flex gap-2.5 items-center bg-vh-warning/8 border border-vh-warning/40 rounded-vh-10 py-2.75 px-3.5 font-semibold text-vh-12-5 font-manrope text-vh-warning">
                <span className="w-2 h-2 bg-vh-warning rounded-full"></span>
                Variação indisponível no momento — escolha outra opção ou
                consulte o atendente.
              </div>
            )}
          </div>
          <div className="flex gap-3 items-center flex-wrap">
            <div className="flex items-center border border-vh-border rounded-vh-10 overflow-hidden">
              <button
                className="vh-qty w-10 h-11 bg-vh-card border-0 text-white font-bold text-vh-18 font-space-grotesk cursor-pointer"
                onClick={() => setQty((n) => Math.max(n - 1, 1))}
              >
                −
              </button>
              <span className="w-12 text-center font-bold text-vh-15 font-space-grotesk">
                {qty}
              </span>
              <button
                className="vh-qty w-10 h-11 bg-vh-card border-0 text-white font-bold text-vh-18 font-space-grotesk cursor-pointer"
                onClick={() => setQty((n) => Math.min(n + 1, 10))}
              >
                +
              </button>
            </div>
            <span className="font-medium text-vh-12-5 font-manrope text-vh-muted">
              Subtotal:{" "}
              <span className="text-vh-lime font-bold text-vh-14 font-space-grotesk">
                {pSubtotalF}
              </span>
            </span>
          </div>
          <div className="flex gap-3 flex-wrap">
            {pCanBuy && (
              <>
                ...
                <a
                  className="vh-wa flex-1 min-w-45 inline-flex items-center justify-center gap-2 bg-vh-wa rounded-vh-11 py-4 px-5 font-bold text-vh-14-5 font-space-grotesk [transition:filter_.15s] text-vh-wa-dark!"
                  href={buyNowUrl}
                  target="_blank"
                >
                  <span className="w-2.25 h-2.25 bg-vh-wa-dark rounded-full"></span>
            ...
            {out && (
              <>
                <button
                  disabled
                  className="flex-1 min-w-45 bg-vh-border text-vh-muted border-0 rounded-vh-11 py-4 px-5 font-bold text-vh-14-5 font-space-grotesk cursor-not-allowed"
                >
                  Produto indisponível
                </button>
                <a
                  className="vh-wa-outline flex-1 min-w-45 inline-flex items-center justify-center bg-transparent border border-vh-wa rounded-vh-11 py-3.75 px-5 font-bold text-vh-14-5 font-space-grotesk text-vh-wa!"
          ...
          <div className="flex flex-col gap-2.25 bg-vh-card border border-vh-border rounded-vh-14 py-4 px-5">
            <span className="font-semibold text-vh-13 font-manrope text-vh-soft">
              <span className="text-vh-lime">✓</span> Garantia:{" "}
              {product.warranty}
            </span>
            <span className="font-semibold text-vh-13 font-manrope text-vh-soft">
              <span className="text-vh-lime">✓</span> Retirada na loja ou
              entrega combinada com o atendente
            </span>
            <span className="font-semibold text-vh-13 font-manrope text-vh-soft">
              <span className="text-vh-lime">✓</span> Produto original e lacrado
            </span>
          </div>
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-5 mt-13">
        <div className="bg-vh-card border border-vh-border rounded-2xl p-6.5">
          <h3 className="mt-0 mx-0 mb-4 font-bold text-vh-17 font-space-grotesk text-vh-accent">
            Especificações técnicas
          </h3>
          {product.specs.map((s, i) => (
            <div
              key={i}
              className="flex justify-between gap-4 py-2.5 px-0 border-b border-b-vh-panel"
            >
              <span className="font-semibold text-vh-13 font-manrope text-vh-muted">
                {s.key}
              </span>
              <span className="font-semibold text-vh-13 font-manrope text-white text-right">
                {s.value}
              </span>
            </div>
          ))}
        ...
        <div className="bg-vh-card border border-vh-border rounded-2xl p-6.5">
          ...
          <p className="m-0 font-medium text-vh-14/vh-17 font-manrope text-vh-soft">
            Retire na loja em Macapá-AP ou combine a entrega diretamente com o
            atendente. Prazos e valores de frete são confirmados no WhatsApp
            antes da compra.
          </p>
        </div>
      </div>
      <h2 className="mt-13 mx-0 mb-5 font-bold text-vh-24 font-space-grotesk">
